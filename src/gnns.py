...
class GNN(torch.nn.Module):
    ...
    def __init__(self, nfeat, nhid=[128], nclass=0, ndim=0, gnn_type='gcn', bias=True, dropout=0.5, batch_norm=False, index=-1):
        ...
        for i in range(self.depth + 2):
            if i == 0:
                ...
                if gnn_type == "gin":
                    gc.append(GINConv(nhid[self.depth-1], ndim))
                elif gnn_type == "gcn":
                    gc.append(GCNConv(nhid[self.depth-1], ndim))
                elif gnn_type == "gat":
                    gc.append(GATConv(nhid[self.depth-1], ndim))
                elif gnn_type == "sage":
                    gc.append(SAGEConv(nhid[self.depth-1], ndim))
            else:
                if gnn_type == "gin":
        ...
        if self.index == -1:
            self.bn = nn.BatchNorm1d(ndim, 0.8)
        elif self.index == -2:
